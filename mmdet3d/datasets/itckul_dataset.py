...
class ITCKULDataset(Det3DDataset):
    r"""ITCKUL Dataset for Detection Task.

    This class serves as the API for experiments on the ITCKUL Dataset.

    Please refer to the `github repo <https://github.com/itckul/itckul>`_
    ...
    def _get_axis_align_matrix(info: dict) -> np.ndarray:
        ...
        if 'axis_align_matrix' in info:
            ...
            warnings.warn(
                'axis_align_matrix is not found in itckul data info, please '
                'use new pre-process scripts to re-generate itckul data')
...
@DATASETS.register_module()
class ITCKULSegDataset(Seg3DDataset):
    r"""ITCKUL Dataset for Semantic Segmentation Task.
...
@DATASETS.register_module()
class ITCKULInstanceSegDataset(Seg3DDataset):
